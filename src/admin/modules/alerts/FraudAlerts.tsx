      ...
      <div style={{ display: 'grid', gap: 16 }}>
        {alerts.length === 0 ? (
          <div style={{ 
            ...
            <h3 style={{ fontSize: 16, color: '#f0e0e4', margin: 0 }}>System Secure</h3>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#f0e0e4' }}>
                      {alert.type}
                    </h3>
                    <span style={{
                      background: '#1a1012', color: '#a09094', fontFamily: 'monospace',
                      padding: '2px 8px', borderRadius: 6, fontSize: 11
                    }}>
                      {alert.rtid}
                    </span>
                  </div>
                  <span style={{ fontSize: 11, color: '#6a5a5d' }}>
                    {alert.timestamp && !isNaN(new Date(alert.timestamp).getTime()) 
                      ? new Date(alert.timestamp).toLocaleString() 
                      : 'Unknown Date'}
                  </span>
                </div>
                <p style={{ margin: 0, fontSize: 13, color: '#c0b0b3', lineHeight: 1.5 }}>
                  {alert.description}
                </p>
              </div>
            </div>
