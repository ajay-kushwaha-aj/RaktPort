    ...
    <div style={{ padding: '32px 36px', maxWidth: 1000, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        {emergencyRequests.length === 0 ? (
          <div style={{ background: '#0f0a0b', border: '1px solid #1e1214', borderRadius: 12, padding: '40px 20px', textAlign: 'center' }}>
            <Siren size={32} color="#6a5a5d" style={{ margin: '0 auto 12px', opacity: 0.3 }} />
            <h3 style={{ fontSize: 16, color: '#e0d0d4', margin: 0 }}>No Critical Emergencies</h3>
          ...
          emergencyRequests.map(req => (
            <div key={req.id} style={{
              background: 'linear-gradient(90deg, rgba(248,113,113,0.05) 0%, #0f0a0b 100%)', 
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 18, fontWeight: 800, color: '#f0e0e4' }}>
                    {req.units} Units Required
                  </span>
                </div>
                
                <h4 style={{ margin: '0 0 12px 0', fontSize: 15, color: '#e0d0d4', fontWeight: 600 }}>
                  {req.hospitalName}
                </h4>
                
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 12, color: '#a09094' }}>
                  ...
                  <span style={{ fontFamily: 'monospace', color: '#6a5a5d' }}>{req.rtid}</span>
                </div>
              </div>

              <button style={{
                background: '#f87171', color: '#0f0a0b', border: 'none', borderRadius: 8,
