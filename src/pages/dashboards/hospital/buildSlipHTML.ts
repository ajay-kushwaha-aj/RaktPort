...
export const buildSlipHTML = (request: BloodRequest, hospital: any, logoDataUrl: string): string => {
  ...
  const rows = (pairs: [string, string][]) =>
    pairs.map(([k, v]) => `<tr><td style="color:#6b7280;width:36%;font-weight:600;padding-right:3pt;vertical-align:top">${k}:</td><td style="font-weight:700;color: var(--rp-text);vertical-align:top">${v || "—"}</td></tr>`).join("");
...
body{font-family:Arial,sans-serif;font-size:10.5pt;color: var(--rp-text);background:#fff;}
...
.serial-val{font-family:monospace;font-size:9.5pt;font-weight:900;color: var(--rp-text);margin-top:1pt;}.serial-dt{font-size:6pt;color:#6b7280;margin-top:1pt;}
...
.sec-head{font-weight:800;font-size:9.5pt;text-transform:uppercase;border-left:3pt solid #ccc;padding-left:5pt;margin-bottom:4pt;color: var(--rp-text);}
...
.blood-box{border:1.5pt solid var(--rp-bg-dark);border-radius:5pt;background:#fafafa;margin-bottom:7pt;}.blood-grid{display:grid;grid-template-columns:repeat(4,1fr);text-align:center;}
...
.bval-md{font-size:10.5pt;font-weight:800;margin-top:4pt;color: var(--rp-text);}.bval-sm{font-size:8.5pt;color:#374151;margin-top:1pt;}
...
.meta-col{flex:1;font-size:7pt;color:#374151;line-height:1.6;}.meta-head{font-weight:800;text-transform:uppercase;font-size:7.5pt;margin-bottom:2pt;color: var(--rp-text);}
...
<script>(function(){try{new QRious({element:document.getElementById('slip-qr'),value:${JSON.stringify(getQRPayload(request))},size:88,foreground: '#C41E3A',level:'H'});}catch(e){}window.addEventListener('load',function(){setTimeout(function(){window.print();setTimeout(function(){window.close();},800);},400);});})();</script>
</body></html>`;
};
