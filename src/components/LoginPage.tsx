    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-100">
        ...
        <div className="w-full max-w-md">
          <div className="bg-[var(--bg-surface)]/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border border-white/30 overflow-hidden">
            ...
            <div className="px-4 sm:px-8 pt-3">
              <div className="flex bg-[var(--bg-page)]/50 rounded-xl p-1 gap-1">
                {([
                    ...
                    <div className="space-y-4 animate-fadein">
                      <div className="space-y-1.5">
                        ...
                        <div className="relative group">
                          <Hash className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] pointer-events-none" />
                          <input
                            type="text"
                            value={smartInput}
                      ...
                      <div className="rounded-xl border-2 border-green-200 bg-green-50/50 p-4 space-y-3">
                        ...
                        <div className="space-y-1.5 text-sm text-gray-700">
                          <div className="flex items-center gap-2">
                            <span className="text-[var(--text-secondary)] w-14 text-xs">Name</span>
                            <span className="font-semibold">{foundUser.fullName}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[var(--text-secondary)] w-14 text-xs">Phone</span>
                            <span className="font-mono font-semibold">{maskPhone(foundUser.phone || '')}</span>
                          </div>
                          {foundUser.internalId && (
                            <div className="flex items-center gap-2">
                              <span className="text-[var(--text-secondary)] w-14 text-xs">ID</span>
                          ...
                          {foundUser.username && (
                            <div className="flex items-center gap-2">
                              <span className="text-[var(--text-secondary)] w-14 text-xs">User</span>
                  ...
                  {flowStep === 'otp' && (
                    <div className="space-y-5 animate-fadein">
                      <div className="flex flex-col items-center text-center gap-2">
                        ...
                        <h3 className="text-sm sm:text-base font-bold text-[var(--text-primary)]">Enter Verification Code</h3>
                        <p className="text-xs text-[var(--text-secondary)]">Sent to {maskPhone(foundUser?.phone || '')}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <button type="button" onClick={resetFlow} disabled={anyBusy}
                          className="text-xs text-[var(--text-secondary)] hover:text-gray-700 font-medium disabled:opacity-50">← Change account</button>
                        <div className="text-xs text-[var(--text-secondary)]">
                          {resendTimer > 0
                            ? <>Resend in <strong className="text-gray-700">{resendTimer}s</strong></>
                            : <button type="button" onClick={handleResendOTP} disabled={anyBusy} className="font-semibold text-gray-700 hover:text-[var(--text-primary)] underline disabled:opacity-50">Resend OTP</button>}
                        </div>
                      </div>
                    </div>
                  )}
              ...
              {tab === 'email' && (
                <form onSubmit={handleEmailLogin} noValidate className="space-y-4 animate-fadein">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      <Mail className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] pointer-events-none" />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      <Lock className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] pointer-events-none" />
                      <input id="lp-password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={passwordInput}
                        ...
                        className="w-full pl-9 sm:pl-12 pr-10 sm:pr-12 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none transition-all text-[var(--text-primary)] placeholder-gray-400 text-sm disabled:opacity-50"
                        placeholder="••••••••" />
                      <button type="button" onClick={() => setShowPassword(v => !v)} tabIndex={-1}
                        className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[var(--text-secondary)] transition-colors p-0.5 touch-manipulation">
      ...
      {showForgotPwd && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--bg-surface)] rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 animate-fadein">
            <h3 className="font-bold text-lg text-[var(--text-primary)]">Reset Password</h3>
            <p className="text-sm text-[var(--text-secondary)]">Enter your email and we'll send a link to reset your password.</p>
            <div className="space-y-1.5">
              <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)}
                placeholder="you@example.com"
                className="w-full px-4 py-3 bg-[var(--bg-surface)] border-2 border-[var(--border-color)] rounded-xl outline-none focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 transition-all text-sm" />
            </div>
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setShowForgotPwd(false)}
                className="px-4 py-2 border-2 border-[var(--border-color)] rounded-xl text-sm font-semibold text-[var(--text-secondary)] hover:bg-[var(--bg-page)]">Cancel</button>
