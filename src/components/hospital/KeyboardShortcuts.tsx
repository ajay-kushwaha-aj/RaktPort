          ...
          {SHORTCUTS.map(s => (
            <div key={s.section}>
              ...
              <div className="space-y-1.5">
                {s.items.map(item => (
                  <div key={item.desc} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-[var(--bg-page)] dark:hover:bg-gray-800/50">
                    <span className="text-xs text-[var(--text-secondary)] dark:text-gray-400 font-medium">{item.desc}</span>
                    <div className="flex items-center gap-1">
                      {item.keys.map((k, i) => (
                        <span key={k}>
                          <kbd className="px-2 py-1 text-[10px] font-bold bg-[var(--bg-page)] dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md border border-[var(--border-color)] dark:border-gray-700 shadow-sm" style={{ fontFamily: "Outfit,monospace" }}>{k}</kbd>
                          {i < item.keys.length - 1 && <span className="text-[10px] text-gray-300 dark:text-[var(--text-secondary)] mx-0.5">+</span>}
                        </span>
                      ))}
                    </div>
                  </div>
