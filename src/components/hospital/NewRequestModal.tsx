        ...
        <div className="sticky top-0 z-10 bg-[var(--bg-surface)] dark:bg-gray-900 rounded-t-2xl border-b border-gray-100 dark:border-gray-800 p-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-bold text-[var(--text-primary)] dark:text-gray-100" style={{ fontFamily: "Outfit,sans-serif" }}>New Blood Requisition</h2>
              <p className="text-xs text-[var(--text-secondary)] dark:text-gray-400">NACO/MoHFW Compliant · {hospitalName}</p>
            </div>
            <button type="button" onClick={() => { if (!isSubmitting) onClose(); }} className="w-8 h-8 rounded-lg bg-[var(--bg-page)] dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center justify-center">
        ...
        <form onSubmit={handleFinalSubmit} className="p-5 space-y-5">
          ...
          <div className="bg-gradient-to-r from-gray-50 dark:from-gray-800/50 to-white dark:to-gray-900 rounded-xl p-4 border border-[var(--border-color)] dark:border-gray-700">
            <div className="flex items-center gap-2 mb-3">
              <Siren className="w-4 h-4 text-[var(--clr-danger)]" />
          ...
          {submitError && (
            <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-[var(--clr-danger)]">
          ...
          {step === 2 && (
            <div className="space-y-4 hd-enter">
              <div className="flex items-center gap-2"><Droplet className="w-4 h-4 text-[var(--clr-danger)]" /><span className="text-sm font-bold text-[var(--text-primary)] dark:text-gray-200">Blood Component Details</span></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="hd-label">Units Required <span className="hd-required">*</span></label>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => setUnitsRequired(u => Math.max(1, u - 1))} className="w-9 h-9 rounded-lg border-2 border-[var(--border-color)] dark:border-gray-700 font-bold text-[var(--text-secondary)] dark:text-gray-400 hover:border-[var(--clr-brand)] transition-colors flex items-center justify-center">−</button>
                    ...
                    <button type="button" onClick={() => setUnitsRequired(u => Math.min(20, u + 1))} className="w-9 h-9 rounded-lg border-2 border-[var(--border-color)] dark:border-gray-700 font-bold text-[var(--text-secondary)] dark:text-gray-400 hover:border-[var(--clr-brand)] transition-colors flex items-center justify-center">+</button>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {TRANSFUSION_INDICATIONS.map(ind => (
                    <button key={ind} type="button" onClick={() => setTransfusionIndication(ind)}
                      className={`py-2 rounded-xl text-[11px] font-semibold border-2 transition-all ${transfusionIndication === ind ? "bg-[var(--clr-info)] text-[var(--txt-inverse)] border-[var(--clr-info)]" : "bg-[var(--bg-page)] dark:bg-gray-800 text-[var(--text-secondary)] dark:text-gray-400 border-[var(--border-color)] dark:border-gray-700 hover:border-blue-300"}`}>
                      {ind}
                    </button>
                  ))}
                </div>
          ...
          {step === 3 && (
            <div className="space-y-4 hd-enter">
              <div className="flex items-center gap-2"><Stethoscope className="w-4 h-4 text-[var(--clr-success)]" /><span className="text-sm font-bold text-[var(--text-primary)] dark:text-gray-200">Doctor & Location Details</span></div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-[var(--bg-page)] dark:bg-gray-800/50 rounded-xl p-4 border border-[var(--border-color)] dark:border-gray-700">
                <p className="text-xs font-bold text-[var(--text-secondary)] dark:text-gray-400 uppercase tracking-wide mb-3">Request Summary</p>
                <div className="grid grid-cols-2 gap-3">
                  {[["Patient", patientName || "—"], ["Age", age ? `${age} yrs` : "—"], ["Blood Group", bloodGroup || "—"], ["Component", componentType], ["Units", String(unitsRequired)], ["Urgency", urgency], ["Valid for", `${uc.validityHours} hours`], ["Indication", transfusionIndication]].map(([k, v]) => (
                    <div key={k}><p className="text-[10px] text-gray-400 font-semibold uppercase">{k}</p><p className="text-xs font-semibold text-[var(--text-primary)] dark:text-gray-200">{v}</p></div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-100 dark:border-gray-800">
            {step > 1 ? (
              <button type="button" onClick={handleBack} className="flex-1 py-2.5 border-2 border-[var(--border-color)] dark:border-gray-700 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-400 hover:bg-[var(--bg-page)] dark:hover:bg-gray-800 transition-all">← Back</button>
            ) : (
              <button type="button" onClick={() => onClose()} className="flex-1 py-2.5 border-2 border-[var(--border-color)] dark:border-gray-700 rounded-xl text-sm font-semibold text-[var(--text-secondary)] dark:text-gray-400 hover:bg-[var(--bg-page)] dark:hover:bg-gray-800 transition-all">Cancel</button>
            )}
            {step < TOTAL ? (
