// hospital/PremiumDashboard.tsx — Overview tab v5.0 with Active Requests section
...
function ActiveRequestCard({
  request, onMarkComplete, onWhatsAppShare, onPrint,
}: {
  ...
  return (
    <div className={`hd-active-card ${isEM ? "emergency-glow" : ""}`}>
      {/* Top urgency strip */}
      <div style={{ height: "3px", background: URGENCY_STRIP[urg] || URGENCY_STRIP.Routine }} />
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: "10px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "7px", flexWrap: "wrap" }}>
              <span style={{
                fontFamily: "var(--f-display)", fontWeight: 700, fontSize: "0.84rem",
                color: "var(--c-text)", overflow: "hidden", textOverflow: "ellipsis",
                whiteSpace: "nowrap", maxWidth: "130px",
              }}>
                {request.patientName}
              </span>
              {request.age && (
                <span style={{ fontSize: "0.66rem", color: "var(--c-text-4)" }}>{request.age}y</span>
              )}
              {/* Blood group pill */}
              <span style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "3px", flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: "0.66rem", color: "var(--c-text-4)" }}>
                {request.componentType || "Whole Blood"} × {request.unitsRequired}u
              </span>
              {request.wardDepartment && (
                <span style={{ fontSize: "0.66rem", color: "var(--c-text-4)" }}>· {request.wardDepartment}</span>
              )}
            </div>
          </div>
          ...
          <div style={{ flexShrink: 0, textAlign: "right" }}>
            ...
            {request.unitsFulfilled > 0 && (
              <div style={{ fontSize: "0.6rem", color: "var(--c-text-4)", marginTop: "3px" }}>
        ...
        {/* Admin progress bar if partially done */}
        {request.unitsAdministered > 0 && request.unitsRequired > 1 && (
        ...
        <div style={{
          ...
          borderTop: "1px solid var(--c-border)", flexWrap: "wrap",
        }}>
          <button onClick={() => onWhatsAppShare(request)} className="hd-share-wa">💬 Share</button>
          <button
            ...
            style={{
              ...
              color: "var(--c-text-3)", fontSize: "0.66rem", fontWeight: 600,
          ...
          {canAdmin && (
            <button
              ...
              style={{
                ...
                color: "var(--c-info)", fontSize: "0.7rem", fontWeight: 700,
                cursor: "pointer", transition: "all var(--t-fast)", fontFamily: "var(--f-body)",
                marginLeft: "auto",
              }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "#DBEAFE"; (e.currentTarget as HTMLElement).style.transform = "translateY(-1px)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-info-bg)"; (e.currentTarget as HTMLElement).style.transform = ""; }}
            >
          ...
          <span style={{ fontSize: "0.6rem", color: "var(--c-text-4)", marginLeft: canAdmin ? "0" : "auto" }}>
...
export function PremiumDashboard({
  requests, hospitalData, kpis, onNewRequest, onViewQR, onDelete,
  onPrint, onMarkComplete, onWhatsAppShare, onExportCSV,
  ...
  /* ── Active requests: everything NOT closed/cancelled/expired/administered ── */
  const activeRequests = useMemo(() =>
    requests
      ...
      .sort((a, b) => {
        const score = { Emergency: 0, Urgent: 1, Routine: 2 };
        const as = score[a.urgency || "Routine"] ?? 2;
        const bs = score[b.urgency || "Routine"] ?? 2;
  ...
  const recentReqs = useMemo(() =>
    [...requests].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0, 5),
    [requests]
  );

  /* ── Blood group distribution ── */
  const bgDist = useMemo(() =>
    BLOOD_GROUPS.reduce((acc: Record<string, number>, bg: string) => {
      acc[bg] = requests.filter((r: BloodRequest) => r.bloodGroup === bg && !["CANCELLED", "EXPIRED"].includes(r.status)).length;
      return acc;
    }, {}),
    [requests]
  );
  const maxBgCount = Math.max(...(Object.values(bgDist) as number[]), 1);

  const fulfillRate = kpis.totalRequests > 0
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div className="hd-welcome hd-enter">
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
            <div>
              <p style={{ fontSize: "0.71rem", color: "rgba(255,255,255,0.5)", fontWeight: 500 }}>
                {greeting} 👋
              </p>
              <h2 className="hd-welcome-title" style={{ marginTop: "4px" }}>
                {(hospitalData?.fullName || "Hospital").toUpperCase()}
              </h2>
              <p style={{ fontSize: "0.7rem", color: "rgba(255,255,255,0.38)", marginTop: "4px" }}>{dateStr}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "14px" }}>
                {activeRequests.length > 0 && (
                  <span style={pillStyle("rgba(255,255,255,0.14)", "rgba(255,255,255,0.9)", "rgba(255,255,255,0.2)")}>
                    <Clock size={11} /> {activeRequests.length} active request{activeRequests.length > 1 ? "s" : ""}
                  </span>
                )}
                <span style={pillStyle("rgba(255,255,255,0.08)", "rgba(255,255,255,0.6)", "rgba(255,255,255,0.15)")}>
                  <MapPin size={11} /> {hospitalData?.district || "…"}, {hospitalData?.pincode || "…"}
                </span>
              </div>
            </div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "flex-start", position: "relative", zIndex: 10 }}>
      ...
      <div>
        <div className="hd-sec-hdr">
          <span className="hd-sec-title">
            <BarChart2 size={16} style={{ color: "var(--c-brand)" }} /> Overview
          </span>
          <span style={{ fontSize: "0.7rem", color: "var(--c-text-4)" }}>{requests.length} total records</span>
        </div>
        <div className="hd-kpi-grid">
          {kpiCards.map((k, i) => (
            <div key={k.label} className={`hd-kpi ${k.cls} hd-enter hd-s${i + 1}`}>
              <span style={{ fontSize: "1.4rem", display: "block", marginBottom: "8px" }}>{k.icon}</span>
          ...
          <span className="hd-sec-title">
            ...
            {activeRequests.length > 0 && (
              <span style={{
                ...
                minWidth: "20px", height: "20px", borderRadius: "var(--r-pill)",
                background: "var(--c-brand)", color: "#fff",
                fontSize: "0.64rem", fontWeight: 800, padding: "0 6px",
              }}>
        ...
        {activeRequests.length === 0 ? (
          <div className="hd-card">
            <div className="hd-empty">
              ...
              <button
                ...
                style={{
                  marginTop: "16px", padding: "8px 18px", borderRadius: "var(--r-md)",
                  background: "var(--c-brand)", color: "#fff", border: "none",
                  fontSize: "0.78rem", fontWeight: 700, cursor: "pointer",
                  fontFamily: "var(--f-body)", display: "inline-flex", alignItems: "center", gap: "6px",
                }}
              >
          ...
          <div style={{
            ...
            {activeRequests.map((r, i) => (
              <div key={r.id} className="hd-enter" style={{ animationDelay: `${i * 0.05}s` }}>
                <ActiveRequestCard
                  request={r}
      ...
      {/* ── Two-column layout: Fulfillment + Right sidebar ── */}
      <div style={{
        display: "grid",
        gridTemplateColumns: "1fr",
        gap: "20px",
      }}>
        <style>{`@media(min-width:1024px){.hd-dash-grid{grid-template-columns:1fr 340px!important}}`}</style>
        <div className="hd-dash-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
          {/* Left: Fulfillment + Recent */}
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div className="hd-card hd-enter hd-s2" style={{ padding: "20px" }}>
              <div className="hd-sec-hdr" style={{ marginBottom: "16px" }}>
                ...
                <span style={{
                  ...
                  fontSize: "1.1rem", color: "var(--c-brand)",
                }}>
                  {fulfillRate}%
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "24px", flexWrap: "wrap" }}>
                ...
                <div style={{ position: "relative", width: "80px", height: "80px", flexShrink: 0 }}>
                  <svg viewBox="0 0 36 36" style={{ width: "80px", height: "80px", transform: "rotate(-90deg)" }}>
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--c-surface-3)" strokeWidth="3.8" />
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--c-brand)" strokeWidth="3.8"
                      strokeDasharray={`${fulfillRate} 100`} strokeLinecap="round"
                  ...
                  <div style={{
                    ...
                    <span style={{
                      fontFamily: "var(--f-display)", fontWeight: 800,
                      fontSize: "0.88rem", color: "var(--c-brand)",
                    }}>
                      {fulfillRate}%
                    </span>
                  </div>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px" }}>
                  {[
                    ...
                    <div key={s.label}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.71rem", marginBottom: "4px" }}>
                        <span style={{ color: "var(--c-text-3)", fontWeight: 500 }}>{s.label}</span>
                        <span style={{ fontWeight: 700, color: "var(--c-text)" }}>{s.val}</span>
                      </div>
                      <div className="hd-prog">
                        <div className="hd-prog-fill" style={{
            ...
            <div className="hd-card hd-enter hd-s3" style={{ overflow: "hidden" }}>
              <div style={{
                ...
                <span className="hd-sec-title">
                  <ClipboardList size={15} style={{ color: "var(--c-brand)" }} /> Recent Requests
                </span>
                <span style={{ fontSize: "0.7rem", color: "var(--c-text-4)" }}>{requests.length} total</span>
              </div>
                ...
                <div className="hd-empty">
                  ...
                  <p className="hd-empty-sub">
                    <button onClick={() => onNewRequest("Routine")} style={{
                      ...
                      cursor: "pointer", fontWeight: 600, fontSize: "0.75rem", fontFamily: "var(--f-body)",
                  ...
                  {recentReqs.map((r, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "7px", flexWrap: "wrap" }}>
                            <span style={{ fontSize: "0.82rem", fontWeight: 650, color: "var(--c-text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "110px" }}>
                              {r.patientName}
                            </span>
                            <span style={{
                              fontFamily: "var(--f-display)", fontWeight: 800, fontSize: "0.64rem",
                          ...
                          <div className="hd-validity" style={{ marginTop: "5px" }}>
                        ...
                        <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                            <button onClick={() => onWhatsAppShare(r)} className="hd-share-wa" title="Share">💬</button>
                            {["REDEEMED", "PARTIAL REDEEMED", "HOSPITAL VERIFIED", "PARTIALLY ADMINISTERED"].includes(r.status) &&
                              ...
                              (r.unitsFulfilled || 0) > (r.unitsAdministered || 0) && (
                                <button
                                  onClick={() => onMarkComplete(r)}
                                  style={{
                                    fontSize: "0.65rem", fontWeight: 700, color: "var(--c-info)",
                                    background: "none", border: "none", cursor: "pointer",
                                    display: "flex", alignItems: "center", gap: "3px", fontFamily: "var(--f-body)",
                                  }}
                                >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div className="hd-card hd-enter hd-s2" style={{ padding: "20px" }}>
              <div className="hd-sec-hdr" style={{ marginBottom: "14px" }}>
                <span className="hd-sec-title">
                  <Droplet size={15} style={{ color: "var(--c-brand)", fill: "var(--c-brand)" }} /> Blood Group Demand
                </span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {BLOOD_GROUPS.filter((bg: string) => bgDist[bg] > 0)
                  .sort((a: string, b: string) => (bgDist[b] || 0) - (bgDist[a] || 0))
                  .map((bg: string) => (
                    <div key={bg} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <span style={{
                      ...
                      <div style={{ flex: 1, height: "6px", background: "var(--c-surface-3)", borderRadius: "var(--r-pill)", overflow: "hidden" }}>
                        <div style={{
                          ...
                          width: `${(bgDist[bg] / maxBgCount) * 100}%`,
                          transition: "width 0.7s ease",
                          transformOrigin: "left",
                          animation: "hd-bar-grow 0.7s ease both",
                        }} />
                      </div>
                      <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--c-text-2)", width: "16px", textAlign: "right" }}>
                        {bgDist[bg]}
                      </span>
                    </div>
                  ))}
                {Object.values(bgDist).every(v => v === 0) && (
                  <p style={{ fontSize: "0.75rem", color: "var(--c-text-4)", textAlign: "center", padding: "16px 0" }}>No requests yet</p>
                )}
              </div>
            </div>
            ...
            <div className="hd-card hd-enter hd-s3" style={{ padding: "20px" }}>
              <div className="hd-sec-hdr" style={{ marginBottom: "14px" }}>
                <span className="hd-sec-title">
                  <Zap size={15} style={{ color: "#D97706" }} /> Quick Actions
                </span>
              </div>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer", textAlign: "left", fontFamily: "var(--f-body)", width: "100%",
                      transition: "all var(--t-med)",
                    }}
                    onMouseEnter={e => {
                      const el = e.currentTarget as HTMLElement;
                      el.style.background = a.accent;
                      el.style.borderColor = "var(--c-border-med)";
                      el.style.transform = "translateY(-1px)";
                      el.style.boxShadow = "var(--s-sm)";
                    }}
                    onMouseLeave={e => {
                      const el = e.currentTarget as HTMLElement;
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "0.79rem", fontWeight: 600, color: "var(--c-text)" }}>{a.label}</div>
                      <div style={{ fontSize: "0.67rem", color: "var(--c-text-4)", marginTop: "1px" }}>{a.sub}</div>
                    </div>
