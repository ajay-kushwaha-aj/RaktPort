...
function getPasswordStrength(pwd: string): { score: number; label: string; color: string } {
  ...
  if (score <= 1) return { score, label: 'Weak', color: 'bg-[var(--clr-danger)]' };
...
function Spinner({ white = false }: { white?: boolean }) {
  return <span className={`inline-block w-4 h-4 border-2 rounded-full animate-spin flex-shrink-0 ${white ? 'border-white/30 border-t-white' : 'border-[var(--border-color)] border-t-gray-600'}`} aria-hidden="true" />;
}
  ...
  return (
    <div>
      <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-1.5">
        {label}{required && <span className="text-[var(--clr-danger)] ml-0.5">*</span>}
      </label>
      {children}
      {hint && <p className="mt-1 text-[10px] sm:text-xs text-[var(--text-secondary)]">{hint}</p>}
    </div>
  );
}
...
function TInput({
  ...
  return (
    <div className="relative group">
      {iconLeft && <span className="absolute left-3 sm:left-3.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[var(--text-secondary)] transition-colors pointer-events-none">{iconLeft}</span>}
      {prefix && <span className={`absolute ${iconLeft ? 'left-9 sm:left-11' : 'left-3'} top-1/2 -translate-y-1/2 text-[var(--text-secondary)] font-medium text-sm pointer-events-none select-none`}>{prefix}</span>}
      <input
        id={id}
        type={type}
        inputMode={inputMode}
        ...
        maxLength={maxLength}
        disabled={disabled}
        className={`w-full ${padL} pr-3 sm:pr-4 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none transition-all text-[var(--text-primary)] placeholder-gray-400 text-sm disabled:opacity-50 disabled:cursor-not-allowed`}
      />
    </div>
  );
}
...
function DocUpload({ inputId, docs, onUpload, onRemove }: {
  ...
  return (
    <Field label="Upload Documents">
      <label
        htmlFor={inputId}
        className="flex flex-col items-center gap-2 px-4 py-5 border-2 border-dashed border-[var(--border-color)] rounded-xl cursor-pointer hover:border-gray-400 transition-colors text-center touch-manipulation"
      >
        <Upload className="w-6 h-6 sm:w-7 sm:h-7 text-gray-400" />
        <span className="text-xs sm:text-sm text-[var(--text-secondary)]">Tap to upload documents</span>
      ...
      {docs.length > 0 && (
        <ul className="mt-2.5 space-y-1.5">
          {docs.map((f, i) => (
            <li key={i} className="flex items-center gap-2 px-3 py-2 bg-[var(--bg-page)] rounded-lg">
              <FileText className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
              <span className="text-xs sm:text-sm text-gray-700 truncate flex-1">{f.name}</span>
              <button type="button" onClick={() => onRemove(i)} className="flex-shrink-0 text-[var(--clr-danger)] hover:text-[var(--clr-danger)] transition-colors touch-manipulation">
                <X className="w-4 h-4" />
              </button>
            </li>
          ))}
...
function InventoryInput({
  ...
  const colors = BLOOD_GROUP_COLORS[bloodGroup] || { bg: 'bg-[var(--bg-page)]', text: 'text-gray-700', border: 'border-[var(--border-color)]' };
  return (
    <div className={`${colors.bg} ${colors.border} border-2 rounded-xl p-3 flex flex-col items-center gap-2`}>
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="w-7 h-7 rounded-lg bg-[var(--bg-surface)]/80 border border-[var(--border-color)] flex items-center justify-center text-[var(--text-secondary)] hover:bg-[var(--bg-page)] active:scale-95 transition-all font-bold text-sm touch-manipulation"
        >−</button>
        <input
          type="number"
          min="0"
          ...
          className={`w-14 text-center py-1 rounded-lg border-2 ${colors.border} bg-[var(--bg-surface)]/80 text-sm font-bold ${colors.text} outline-none focus:ring-2 focus:ring-purple-300`}
        />
        <button
          type="button"
...
function GoogleProfileCompletion({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--bg-surface)] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className={`p-5 bg-gradient-to-r ${cfg.gradient} text-[var(--txt-inverse)] rounded-t-2xl`}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[var(--bg-surface)]/20 flex items-center justify-center">
              <GoogleIcon size={20} />
            </div>
            <div>
          ...
          <div className="flex gap-2 mt-4">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} className={`flex-1 h-1.5 rounded-full transition-all ${i < step ? 'bg-[var(--bg-surface)]' : 'bg-[var(--bg-surface)]/30'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fadein">
              <h3 className="font-bold text-[var(--text-primary)]">Organization Details</h3>
              {role === 'hospital' && (
          ...
          {step === 2 && role === 'bloodbank' && (
            <div className="space-y-4 animate-fadein">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-[var(--text-primary)]">Initial Blood Inventory</h3>
              </div>
              <p className="text-xs text-[var(--text-secondary)]">Enter current stock levels (you can update later from dashboard)</p>
              ...
              <Field label="Complete Address" required>
                <div className="relative">
                  ...
                  <textarea rows={2} value={address} onChange={e => setAddress(e.target.value)}
                    ...
                    className="w-full pl-9 pr-3 py-3 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none text-sm text-[var(--text-primary)] placeholder-gray-400 resize-none transition-all" />
                </div>
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="District" required>
                  <input value={district} onChange={e => setDistrict(e.target.value)} placeholder="District"
                    className="w-full px-3 py-3 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none text-sm text-[var(--text-primary)] placeholder-gray-400 transition-all" />
                </Field>
                <Field label="State" required>
                  <select value={state} onChange={e => setState(e.target.value)}
                    className="w-full px-3 py-3 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none text-sm text-[var(--text-primary)] transition-all">
          ...
          <div className="flex gap-3 pt-2">
            {step > 1 ? (
              <button type="button" onClick={() => setStep(s => s - 1)}
                className="px-4 py-2.5 bg-[var(--bg-surface)] border-2 border-[var(--border-color)] rounded-xl text-sm font-semibold text-gray-700 hover:bg-[var(--bg-page)] active:scale-95 transition-all">
                ← Back
              </button>
            ) : (
              <button type="button" onClick={onCancel}
                className="px-4 py-2.5 bg-[var(--bg-surface)] border-2 border-[var(--border-color)] rounded-xl text-sm font-semibold text-[var(--text-secondary)] hover:bg-[var(--bg-page)] active:scale-95 transition-all">
                Cancel
              </button>
            )}
            {step < totalSteps ? (
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden bg-gradient-to-br ${cfg.bgGrad}`}>
      ...
      <div className="sticky top-0 z-30 flex items-center px-3 pt-3 pb-2 sm:px-6 sm:pt-5 sm:pb-0 pointer-events-none">
        <button onClick={onBack} className="pointer-events-auto inline-flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 bg-[var(--bg-surface)]/85 backdrop-blur-md rounded-full shadow-md hover:shadow-lg hover:bg-[var(--bg-surface)] active:scale-95 transition-all touch-manipulation">
          ...
          <div className="bg-[var(--bg-surface)]/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border border-white/30 overflow-hidden">
            ...
            <div className="pt-5 pb-4 px-4 sm:pt-8 sm:pb-6 sm:px-8 bg-gradient-to-br from-white/50 to-white/20 flex flex-col items-center text-center">
              ...
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <h1 className="text-lg sm:text-2xl font-bold text-[var(--text-primary)]">{cfg.title}</h1>
              </div>
              <p className="text-[11px] sm:text-sm text-[var(--text-secondary)]">{cfg.subtitle}</p>
            </div>
            ...
            <div className="px-4 sm:px-8 py-5 sm:py-6">
              <form noValidate className="space-y-5">
                ...
                {step === 1 && (
                  <div className="space-y-4 animate-fadein">
                    <h2 className="text-base sm:text-xl font-bold text-[var(--text-primary)]">Basic Information</h2>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2.5 min-h-[50px] px-4 py-3 sm:py-3.5 bg-[var(--bg-surface)] border-2 border-[var(--border-color)] rounded-xl hover:border-[var(--border-color)] hover:shadow-md active:scale-[0.98] transition-all font-semibold text-sm text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation"
                    >
                    ...
                    >
                      <div className="relative group">
                        <AtSign className="absolute left-3 sm:left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] transition-colors pointer-events-none" />
                        <input
                          type="text"
                          value={form.username}
                          ...
                          maxLength={20}
                          className="w-full pl-9 sm:pl-11 pr-24 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none transition-all text-[var(--text-primary)] placeholder-gray-400 text-sm"
                        />
                      ...
                      {form.username.length >= 3 && (
                        <div className={`mt-1.5 flex items-center gap-1.5 text-[11px] font-medium ${
                          usernameStatus === 'checking'  ? 'text-[var(--text-secondary)]' :
                          usernameStatus === 'available' ? 'text-[var(--clr-success)]' :
                          usernameStatus === 'taken'     ? 'text-[var(--clr-danger)]' :
                          usernameStatus === 'invalid'   ? 'text-amber-600' : 'text-[var(--text-secondary)]'
                        }`}>
                ...
                {step === 2 && (
                  <div className="space-y-5 animate-fadein">
                    <div className="flex flex-col items-center text-center gap-3">
                      ...
                      <div>
                        <h2 className="text-base sm:text-xl font-bold text-[var(--text-primary)] mb-1">Verify Email Address</h2>
                            ...
                            <input
                              ...
                              className={`text-center text-xl sm:text-2xl font-bold border-2 rounded-xl outline-none transition-all touch-manipulation ${
                                otpVerified
                                  ...
                                  : digit
                                    ? 'bg-[var(--bg-surface)] border-gray-500'
                                    : 'bg-[var(--bg-surface)]/60 border-[var(--border-color)] focus:border-gray-500 focus:ring-4 focus:ring-gray-200/50'
                              }`}
                          ...
                          <>
                            ...
                            <p className="text-center text-xs sm:text-sm text-[var(--text-secondary)]">
                              {resendTimer > 0
                                ...
                                : <button type="button" onClick={async () => { setOtp(['','','','','','']); await sendOTP(); }} className="font-semibold text-gray-700 hover:text-[var(--text-primary)] underline touch-manipulation">Resend OTP</button>}
                            </p>
                          </>
                        )}
                      </div>
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <Field label="Blood Group" required>
                            <select value={form.bloodGroup} onChange={e => set('bloodGroup', e.target.value)}
                              className="w-full px-3 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none text-sm text-[var(--text-primary)] transition-all">
                              <option value="">Select</option>
                        ...
                        <Field label="Last Donation Date" hint="Leave blank if you haven't donated before">
                          ...
                          <label className="flex items-center gap-2 mt-2 cursor-pointer touch-manipulation">
                            <input type="checkbox" className="w-4 h-4 rounded border-[var(--border-color)] accent-gray-700" checked={form.dontRemember}
                              onChange={e => { set('dontRemember', e.target.checked); if (e.target.checked) set('lastDonationDate', ''); }} />
                            <span className="text-xs sm:text-sm text-[var(--text-secondary)]">I don't remember</span>
                          </label>
                        </Field>
                      </>
                    )}
                  ...
                  <div className="space-y-4 animate-fadein">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-base sm:text-xl font-bold text-[var(--text-primary)]">Initial Blood Inventory</h2>
                      </div>
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-[var(--text-secondary)] font-medium">Quick presets:</p>
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-semibold bg-[var(--bg-surface)] border-2 border-purple-200 text-purple-700 rounded-xl hover:bg-purple-50 active:scale-95 transition-all touch-manipulation"
                          >
                            {preset.label}
                          </button>
                        ))}
                ...
                {((step === 4 && role !== 'bloodbank') || (step === 5 && role === 'bloodbank')) && (
                  <div className="space-y-4 animate-fadein">
                    ...
                    <Field label="Complete Address" required>
                      <div className="relative group">
                        <Home className="absolute left-3 sm:left-3.5 top-3 sm:top-3.5 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] transition-colors pointer-events-none" />
                        <textarea rows={3} value={form.address} onChange={e => set('address', e.target.value)}
                          placeholder="Street, Building, Landmark…"
                          className="w-full pl-9 sm:pl-11 pr-3 sm:pr-4 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none transition-all resize-none text-sm text-[var(--text-primary)] placeholder-gray-400" />
                      </div>
                    </Field>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="District" required>
                        <input value={form.district} onChange={e => set('district', e.target.value)} placeholder="District"
                          className="w-full px-3 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none text-sm text-[var(--text-primary)] placeholder-gray-400 transition-all" />
                      </Field>
                      <Field label="State" required>
                ...
                {step === TOTAL_STEPS && (
                  <div className="space-y-4 animate-fadein">
                    <h2 className="text-base sm:text-xl font-bold text-[var(--text-primary)]">Set Password &amp; Finish</h2>

                    <Field label="Password" required>
                      <div className="relative group">
                        <Lock className="absolute left-3 sm:left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-[var(--text-secondary)] transition-colors pointer-events-none" />
                        <input type={showPassword ? 'text' : 'password'} value={form.password}
                          onChange={e => set('password', e.target.value)} placeholder="••••••••"
                          className="w-full pl-9 sm:pl-11 pr-10 sm:pr-12 py-3 sm:py-3.5 bg-[var(--bg-surface)]/60 border-2 border-[var(--border-color)] rounded-xl focus:border-gray-400 focus:ring-4 focus:ring-gray-200/50 outline-none transition-all text-sm text-[var(--text-primary)] placeholder-gray-400" />
                        <button type="button" tabIndex={-1} onClick={() => setShowPassword(v => !v)} aria-label={showPassword ? 'Hide' : 'Show'}
                          className="absolute right-3 sm:right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[var(--text-secondary)] transition-colors touch-manipulation p-0.5">
                      ...
                      {form.password.length > 0 && (
                        <div className="mt-2 space-y-1">
                          ...
                          <p className={`text-[10px] font-semibold ${
                            ...
                            pwdStrength.label === 'Fair'   ? 'text-amber-600' : 'text-[var(--clr-danger)]'
                          }`}>
                    ...
                    <Field label="Confirm Password" required>
                      ...
                      {form.confirmPassword.length > 0 && (
                        <p className={`mt-1 text-[11px] font-medium flex items-center gap-1 ${form.password === form.confirmPassword ? 'text-[var(--clr-success)]' : 'text-[var(--clr-danger)]'}`}>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer touch-manipulation">
                      <span className="relative flex-shrink-0 mt-0.5">
                        ...
                        <span className="flex w-5 h-5 border-2 border-[var(--border-color)] rounded-md peer-checked:bg-gradient-to-br peer-checked:from-gray-700 peer-checked:to-gray-900 peer-checked:border-gray-700 transition-all items-center justify-center">
                          {form.acceptTerms && <CheckCircle2 className="w-3 h-3 text-[var(--txt-inverse)]" />}
                        </span>
                      </span>
                      <span className="text-xs sm:text-sm text-gray-700 leading-relaxed">
                        I agree to the <strong className="text-[var(--text-primary)]">Terms &amp; Conditions</strong> and <strong className="text-[var(--text-primary)]">Privacy Policy</strong> of RaktPort
                      </span>
                    </label>

                    {role === 'donor' && (
                      <div className={`flex gap-2.5 p-3 sm:p-4 ${cfg.lightBg} rounded-xl border-l-4 ${cfg.borderL}`}>
                        <Heart className="w-4 h-4 flex-shrink-0 mt-0.5 text-[var(--clr-danger)]" />
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 gap-3">
                  {step > 1 && (
                    <button type="button" onClick={goPrev} disabled={loading}
                      className="flex items-center gap-1.5 px-4 sm:px-5 py-2.5 sm:py-3 min-h-[46px] bg-[var(--bg-surface)] border-2 border-[var(--border-color)] rounded-xl hover:bg-[var(--bg-page)] active:scale-95 transition-all font-semibold text-sm text-gray-700 disabled:opacity-50 touch-manipulation">
